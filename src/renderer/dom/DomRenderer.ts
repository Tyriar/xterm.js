import { IRenderer, IRenderDimensions, IColorManager, IColorSet } from '../Interfaces';
import { EventEmitter } from '../../EventEmitter';
import { ITheme, IListenerType, ITerminal } from '../../Interfaces';
...
export class DomRenderer extends EventEmitter implements IRenderer {
  ...
  constructor(private _terminal: ITerminal, theme: ITheme) {
    ...
    this._terminal.element.appendChild(this._rowContainer);
  ...
  public onResize(cols: number, rows: number, didCharSizeChange: boolean): void {
    while (this._children.length < rows) {
      const row = document.createElement('div');
      this._rowContainer.appendChild(row);
      this._children.push(row);
    }
    while (this._children.length > rows) {
      const el = this._children.shift();
      if (!el) continue;
      el.parentNode.removeChild(el);
    }

    this.dimensions.scaledCharWidth = this._terminal.charMeasure.width;
    this.dimensions.scaledCharHeight = this._terminal.charMeasure.height;
    ...
    this.dimensions.canvasHeight = this.dimensions.scaledCanvasHeight * this._terminal.rows;
    this.dimensions.canvasWidth = this.dimensions.scaledCanvasWidth * this._terminal.cols;
    this.dimensions.actualCellHeight = this.dimensions.scaledCharHeight;
    this.dimensions.actualCellWidth = this.dimensions.scaledCharWidth;
  }
  ...
  private _refresh(start: number, end: number): void {
    ...
    if (end - start >= this._terminal.rows / 2) {
      ...
      if (parent) {
        this._terminal.element.removeChild(this._rowContainer);
      }
    }
    ...
    for (; y <= end; y++) {
      ...
      while (this._children[y].children.length) {
        const child = this._children[y].children[0];
        this._children[y].removeChild(child);
      ...
      this._children[y].appendChild(documentFragment);
    }
